const get_election_list_html = (name, prompt, privateKey, endDate, userID, yesVotes, noVotes) => {
    ...
    html += "<h5> Election Private Key:</h5>";
    html += "<h5 class='small text-success'>" + privateKey + "</h5><br>";
    html += "<h5> Election userID:</h5>";
    html += "<h5 class='small text-success'>" + userID + "</h5><br>";
    html += "<h5> Number of Yes Votes:</h5>";
...
const noElection = () => {
    var html = "<h5>You have no elections to display.</h5>";
    return html;
}
...
fetch('/get_my_elections', {
    ...
    .then(data => {
        my_election_list = document.getElementById("my_election_list");
        if (data.length == 0) {
